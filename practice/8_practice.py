    ...
    for read_file_idx in read_file:
        ...
        for proc_idx in except_proc:
            ...
            if read_except_proc not in read_file_idx : #compare str and str
                ...
                print ("##### EX_PROCESS #####"+read_except_proc)
                read_file.remove(read_file_idx)
                break

            elif read_except_proc not in read_file_idx :
                print ("##### NO EX_IP #####\n")
                for ip_idx in except_ip:
                    read_except_ip = (" "+ip_idx+":")
                    if read_except_ip in read_file_idx :
                        print ("##### EX_IP #####"+read_file_idx)
                        read_file.remove(read_file_idx)
                        print ("##### EX_IP #####"+read_except_port)
                        break

                    elif read_except_ip not in read_file_idx :
                        print ("##### NO EX_PORT #####\n")
                        for port_idx in except_port:
                            read_except_port = (":"+port_idx+" ")
                            if read_except_port in read_file_idx:
                                print ("##### EX_PORT #####"+read_file_idx)
                                read_file.remove(read_file_idx)
                                print ("##### EX_PORT #####"+read_except_port)
                                break
